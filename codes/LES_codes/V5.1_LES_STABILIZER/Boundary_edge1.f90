        ...
        LX1 = (u_cur(i,j,k) - c) *(p_x(i,j,k) -rho_cur(i,j,k)*c*u_x(i,j,k))
        end if
        ...
        LX5 = (u_cur(i,j,k)+c)*(p_x(i,j,k)+rho_cur(i,j,k)*c*u_x(i,j,k))
        end if
        ...
        LY1 = (v_cur(i,j,k) - c)*(p_y(i,j,k) - rho_cur(i,j,k)*c*v_y(i,j,k))
        end if
        ...
        LY5 = (v_cur(i,j,k)+c)*(p_y(i,j,k)+rho_cur(i,j,k)*c*v_y(i,j,k))
        end if


        drho1(i,j,k) = (LX2+0.5d0*(LX5+LX1))/(c*c) + (LY2+0.5d0*(LY5+LY1))/(c*c) 
        drho1(i,j,k) = drho1(i,j,k) + w_cur(i,j,k)*rho_z(i,j,k)
        drho1(i,j,k) = drho1(i,j,k) + rho_cur(i,j,k)*w_z(i,j,k)

        du = 0.5d0*(LX5-LX1)/(rho_cur(i,j,k)*c) + LY3 + w_cur(i,j,k)*u_z(i,j,k)

        dv = 0.5d0*(LY5-LY1)/(rho_cur(i,j,k)*c) + LX3 + w_cur(i,j,k)*v_z(i,j,k)

        dw = LX4 + LY4 + p_z(i,j,k)/rho_cur(i,j,k) + w_cur(i,j,k)*w_z(i,j,k)

        dp = (LX5 + LX1)*0.5d0 + (LY5 + LY1)*0.5d0 
        dp = dp + w_cur(i,j,k)*p_z(i,j,k)+ gama*p_cur(i,j,k)*w_z(i,j,k)
        ...
        LZ1 = (w_cur(i,j,k) - c)*(p_z(i,j,k) - rho_cur(i,j,k)*c*w_z(i,j,k))
        end if
        ...
        LZ5 = (w_cur(i,j,k)+c)*(p_z(i,j,k) + rho_cur(i,j,k)*c*w_z(i,j,k))
        end if


        drho1(i,j,k) = (LX2+0.5d0*(LX5+LX1))/(c*c) + (LZ2+0.5d0*(LZ5+LZ1))/(c*c) 
        drho1(i,j,k) = drho1(i,j,k) + v_cur(i,j,k)*rho_y(i,j,k)
        drho1(i,j,k) = drho1(i,j,k) + rho_cur(i,j,k)*v_y(i,j,k)

        du = 0.5d0*(LX5-LX1)/(rho_cur(i,j,k)*c) + LZ3 + v_cur(i,j,k)*u_y(i,j,k)

        dw = 0.5d0*(LZ5-LZ1)/(rho_cur(i,j,k)*c) + LX4 + v_cur(i,j,k)*w_y(i,j,k)

        dv = LX3 + LZ4 + p_y(i,j,k)/rho_cur(i,j,k) + v_cur(i,j,k)*v_y(i,j,k)

        dp = (LX5 + LX1)*0.5d0 + (LZ5 + LZ1)*0.5d0 
        dp = dp + v_cur(i,j,k)*p_y(i,j,k) + gama*p_cur(i,j,k)*v_y(i,j,k)
        ...
        dw = LX4 + 0.5d0*(LZ5-LZ1)/(rho_cur(i,j,k)*c) + v_cur(i,j,k)*w_y(i,j,k)
        ...
        drho1(i,j,k) = (LX2+0.5d0*(LX5+LX1))/(c*c) + (LY2+0.5d0*(LY5+LY1))/(c*c) 
        drho1(i,j,k) = drho1(i,j,k) + w_cur(i,j,k)*rho_z(i,j,k) + rho_cur(i,j,k)*w_z(i,j,k)

        dv = 0.5d0*(LY5-LY1)/(rho_cur(i,j,k)*c) + LX3 + w_cur(i,j,k)*v_z(i,j,k)

        du = 0.5d0*(LX5-LX1)/(rho_cur(i,j,k)*c) + LY3 + w_cur(i,j,k)*u_z(i,j,k)

        dw = LY4 + p_z(i,j,k)/rho_cur(i,j,k) + LX4 + w_cur(i,j,k)*w_z(i,j,k)
        ...
        drho1(i,j,k) = (LX2+0.5d0*(LX5+LX1))/(c*c) + (LZ2+0.5d0*(LZ5+LZ1))/(c*c) 
        drho1(i,j,k) = drho1(i,j,k) + v_cur(i,j,k)*rho_y(i,j,k) + rho_cur(i,j,k)*v_y(i,j,k)
        ...
        drho1(i,j,k) = (LY2+0.5d0*(LY5+LY1))/(c*c) + (LZ2+0.5d0*(LZ5+LZ1))/(c*c)
        drho1(i,j,k) = drho1(i,j,k) + rho_cur(i,j,k)*u_x(i,j,k) + u_cur(i,j,k)*rho_x(i,j,k) 

        dv = 0.5d0*(LY5-LY1)/(rho_cur(i,j,k)*c) + LZ4 + u_cur(i,j,k)*v_x(i,j,k)

        du = LY3 + LZ3 + p_x(i,j,k)/rho_cur(i,j,k) + u_cur(i,j,k)*u_x(i,j,k)

        dw = 0.5d0*(LZ5-LZ1)/(rho_cur(i,j,k)*c) + LY4  + u_cur(i,j,k)*w_x(i,j,k)
 
        dp = (LY5 + LY1)*0.5d0 + (LZ5 + LZ1)*0.5d0 
        dp = dp + gama*p_cur(i,j,k)*u_x(i,j,k) + u_cur(i,j,k)*p_x(i,j,k)
