	...
	REAL (KIND=8) :: m1,m2,m3
        ...
        !write(*,*) 'Differentiation'
                ...
                !!CONTINUITY EQUATION
                !rhou_x = rhou_cur !CALL COPY(rhou_cur,rhou_x)
                !rhov_y = rhov_cur !CALL COPY(rhov_cur,rhov_y)
                !rhow_z = rhow_cur !CALL COPY(rhow_cur,rhow_z)
                !!CALL COPY(rho_cur,rho_x)
                !!CALL COPY(rho_cur,rho_y)
                !!CALL COPY(rho_cur,rho_z)
        ...
        CALL PRODUCT_OF_TWO_TERMS(rhou_cur,u_cur,rhouu_x)
        !CALL DIVIDE(rho_cur,rhouu_x)
        CALL PRODUCT_OF_TWO_TERMS(rhou_cur,v_cur,rhouv_y)
        !CALL DIVIDE(rho_cur,rhouv_y)
        CALL PRODUCT_OF_TWO_TERMS(rhou_cur,w_cur,rhouw_z)
        !CALL DIVIDE(rho_cur,rhouw_z)
        CALL COPY(p_cur,p_x)

        !Y_MOMENTUM CONVECTIVE TERMS
        CALL COPY(rhouv_y,rhovu_x)
        CALL PRODUCT_OF_TWO_TERMS(rhov_cur,v_cur,rhovv_y)
        !CALL DIVIDE(rho_cur,rhovv_y)
        CALL PRODUCT_OF_TWO_TERMS(rhov_cur,w_cur,rhovw_z)
        !CALL DIVIDE(rho_cur,rhovw_z)
        CALL COPY(p_cur,p_y)

       !Z_MOMENTUM CONVECTIVE TERMS
        CALL COPY(rhouw_z,rhowu_x)
        CALL COPY(rhovw_z,rhowv_y)
        CALL PRODUCT_OF_TWO_TERMS(rhow_cur,w_cur,rhoww_z)
        !CALL DIVIDE(rho_cur,rhoww_z)
        CALL COPY(p_cur,p_z)

        !ENERGY EQUATION CONVECTIVE TERMS
        CALL SUM_OF_TWO_TERMS(e_cur,p_cur,efl_x)
        ...
        !CONTINUITY EQUATION
        CALL TURKEL_FORWARD_X(rhou_cur,rhou_x)
        CALL TURKEL_FORWARD_X(rho_cur,rho_x)
        !MOMENTUM EQUATION VISCOUS TERMS
        ...
        !CONTINUITY EQUATION
        CALL TURKEL_FORWARD_Y(rhov_cur,rhov_y)
        CALL TURKEL_FORWARD_Y(rho_cur,rho_y)
        !MOMENTUM EQUATION VISCOUS TERMS
        ...
        !CONTINUITY EQUATION
        CALL TURKEL_FORWARD_Z(rhow_cur,rhow_z)
        CALL TURKEL_FORWARD_Z(rho_cur,rho_z)
        !MOMENTUM EQUATION VISCOUS TERMS
        ...
        !CONTINUITY EQUATION
        CALL TURKEL_BACKDIF_X(rhou_cur,rhou_x)
        CALL TURKEL_BACKDIF_X(rho_cur,rho_x)
        !MOMENTUM EQUATION VISCOUS TERMS
        ...
        !CONTINUITY EQUATION
        CALL TURKEL_BACKDIF_Y(rhov_cur,rhov_y)
        CALL TURKEL_BACKDIF_Y(rho_cur,rho_y)
        !MOMENTUM EQUATION VISCOUS TERMS
        ...
        !CONTINUITY EQUATION
        CALL TURKEL_BACKDIF_Z(rhow_cur,rhow_z)
        CALL TURKEL_BACKDIF_Z(rho_cur,rho_z)
        !MOMENTUM EQUATION VISCOUS TERMS
        ...
        drho1(i,j,k) = rhou_x(i,j,k) + rhov_y(i,j,k) + rhow_z(i,j,k)
  
        drhou1(i,j,k) = rhouu_x(i,j,k) + rhouv_y(i,j,k) + rhouw_z(i,j,k)
        drhou1(i,j,k) = drhou1(i,j,k) + p_x(i,j,k)
        drhou1(i,j,k) = drhou1(i,j,k) - (vrhou_xx(i,j,k)+vrhou_yy(i,j,k)+vrhou_zz(i,j,k))
      
        drhov1(i,j,k) = rhovu_x(i,j,k) + rhovv_y(i,j,k) + rhovw_z(i,j,k)
        drhov1(i,j,k) = drhov1(i,j,k) + p_y(i,j,k)
        drhov1(i,j,k) = drhov1(i,j,k) - (vrhov_xx(i,j,k)+vrhov_yy(i,j,k)+vrhov_zz(i,j,k))

        drhow1(i,j,k) = rhowu_x(i,j,k) + rhowv_y(i,j,k) + rhoww_z(i,j,k)
        drhow1(i,j,k) = drhow1(i,j,k) + p_z(i,j,k)
        drhow1(i,j,k) = drhow1(i,j,k) - (vrhow_xx(i,j,k)+vrhow_yy(i,j,k)+vrhow_zz(i,j,k))

        de1(i,j,k) = efl_x(i,j,k) + efl_y(i,j,k) + efl_z(i,j,k)
        de1(i,j,k) = de1(i,j,k) - (vke_xx(i,j,k)+vke_yy(i,j,k)+vke_zz(i,j,k))
