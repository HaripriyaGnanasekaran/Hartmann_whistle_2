        ...
        L1 = (v_cur(i,j,k) - c)*(p_y(i,j,k) - rho_int(i,j,k)*c*v_y(i,j,k))
        end if
        ...
        L5 = (v_cur(i,j,k)+c)*(p_y(i,j,k)+rho_int(i,j,k)*c*v_y(i,j,k))
        end if

        drho2(i,j,k) = (L2+0.5d0*(L5+L1))/(c*c) + u_cur(i,j,k)*rho_x(i,j,k) 
        drho2(i,j,k) = drho2(i,j,k) + rho_int(i,j,k)*u_x(i,j,k)
        drho2(i,j,k) = drho2(i,j,k) + w_cur(i,j,k)*rho_z(i,j,k)
        drho2(i,j,k) = drho2(i,j,k) + rho_int(i,j,k)*w_z(i,j,k)

        dv = 0.5d0*(L5-L1)/(rho_int(i,j,k)*c) + u_cur(i,j,k)*v_x(i,j,k)
        dv = dv + w_cur(i,j,k)*v_z(i,j,k)

        du = L3 + p_x(i,j,k)/rho_int(i,j,k) + u_cur(i,j,k)*u_x(i,j,k)
        du = du + w_cur(i,j,k)*u_z(i,j,k)

        dw = L4 + p_z(i,j,k)/rho_int(i,j,k) + u_cur(i,j,k)*w_x(i,j,k)
        dw = dw + w_cur(i,j,k)*w_z(i,j,k)

        dp = (L5 + L1)*0.5d0 + u_cur(i,j,k)*p_x(i,j,k) 
        dp = dp + gama*p_int(i,j,k)*u_x(i,j,k)
        ...
        L1 = (u_cur(i,j,k) - c) *(p_x(i,j,k) -rho_int(i,j,k)*c*u_x(i,j,k))
        end if
        ...
        L5 = (u_cur(i,j,k)+c)*(p_x(i,j,k)+rho_int(i,j,k)*c*u_x(i,j,k))
        end if

        drho2(i,j,k) = (L2 + 0.5d0*(L1+L5))/(c*c) + v_cur(i,j,k)*rho_y(i,j,k)
        drho2(i,j,k) = drho2(i,j,k) + rho_int(i,j,k)*v_y(i,j,k)
        drho2(i,j,k) = drho2(i,j,k) + w_cur(i,j,k)*rho_z(i,j,k)
        drho2(i,j,k) = drho2(i,j,k) + rho_int(i,j,k)*w_z(i,j,k)

        du = 0.5d0*(L5-L1)/(rho_int(i,j,k)*c) + v_cur(i,j,k)*u_y(i,j,k)
        du = du + w_cur(i,j,k)*u_z(i,j,k)

        dv = L3 + p_y(i,j,k)/rho_int(i,j,k)+ v_cur(i,j,k)*v_y(i,j,k)
        dv = dv + w_cur(i,j,k)*v_z(i,j,k)

        dw = L4 + p_z(i,j,k)/rho_int(i,j,k)+ v_cur(i,j,k)*w_y(i,j,k)
        dw = dw + w_cur(i,j,k)*w_z(i,j,k)

        dp = 0.5d0*(L5 + L1) + v_cur(i,j,k)*p_y(i,j,k)
        dp = dp + gama*p_int(i,j,k)*v_y(i,j,k)
        dp = dp + w_cur(i,j,k)*p_z(i,j,k)
        dp = dp + gama*p_int(i,j,k)*w_z(i,j,k)
        ...
        L1 = (w_cur(i,j,k) - c)*(p_z(i,j,k) - rho_int(i,j,k)*c*w_z(i,j,k))
        end if
        ...
        L5 = (w_cur(i,j,k)+c)*(p_z(i,j,k) + rho_int(i,j,k)*c*w_z(i,j,k))
        end if
        ...
        dw = 0.5d0*(L5-L1)/(rho_int(i,j,k)*c)+u_cur(i,j,k)*w_x(i,j,k)
        dw = dw + v_cur(i,j,k)*w_y(i,j,k)

        du = L3 + p_x(i,j,k)/rho_int(i,j,k) + u_cur(i,j,k)*u_x(i,j,k)
        du = du + v_cur(i,j,k)*u_y(i,j,k)

        dv = L4 + p_y(i,j,k)/rho_int(i,j,k) + u_cur(i,j,k)*v_x(i,j,k)
        ...
        !write(*,*) 'Deallocated int variables'
